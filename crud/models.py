...
class Users(models.Model):
    ...
    gender = models.ForeignKey(Genders, on_delete=models.CASCADE) # ender_id BIGINT NOT NULL // FOREIGN KEY(gender_id) REFERENCES tbl_genders(gender_Id) ON DELETE CASCADE
    ...
    email = models.EmailField(max_length=55, blank=False) # email VARCHAR (55) DEFAULT NULL
    username = models.CharField(max_length=55, blank=False, unique=True) # username VARCHAR (55) NOT NULL UNIQUE
    password = models.CharField(max_length=225, blank=False) # password VARCHAR (255) NOT NULL
    created_at = models.DateTimeField(auto_now_add=True) #created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    updated_at = models.DateTimeField(auto_now=True) # updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP